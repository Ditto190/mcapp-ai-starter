# Code of Conduct for AgentSkills

## Our Pledge

We are committed to providing a welcoming and inclusive environment for everyone, regardless of:
- Background, education, or experience level
- Gender identity and expression
- Sexual orientation
- Disability
- Physical appearance
- Body size
- Race, ethnicity, age, religion, or nationality
- Any other characteristic

## Our Standards

### ✅ Positive Behavior

- **Be respectful and professional** in all interactions
- **Be inclusive** and welcome diverse perspectives
- **Be constructive** in feedback and criticism
- **Be collaborative** and help others succeed
- **Be accountable** for your words and actions
- **Focus on what is best for the community**
- **Show empathy** towards other community members

### ❌ Unacceptable Behavior

- **Harassment**, sexualized language, or unwelcome attention
- **Trolling**, insulting or derogatory comments
- **Personal or political attacks**
- **Public or private harassment**
- **Publishing private information** without permission
- **Any other conduct** that could reasonably be considered inappropriate in a professional setting

## Scope

This Code of Conduct applies to all spaces managed by the AgentSkills project, including:
- **GitHub repositories** (issues, PRs, discussions)
- **Slack channels** and other communication platforms
- **Meetings and events** (virtual or in-person)
- **Private communications** when related to project work

## Enforcement

### Reporting Incidents

If you experience or witness unacceptable behavior, please report it:

**Primary Contact**: 
- **Email**: conduct@yourcompany.com
- **Slack**: Direct message any Trusted Committer
- **GitHub**: Report issue to any repository maintainer

**What to Include**:
- **Description** of the incident
- **People involved** (if known)
- **Date and time** of the incident
- **Screenshots or logs** (if applicable)
- **Your preferred resolution** (if any)

### Response Process

1. **Immediate Response**: Acknowledge receipt within 24 hours
2. **Investigation**: Review the incident thoroughly and privately
3. **Action**: Take appropriate corrective action
4. **Follow-up**: Inform reporter of resolution (if requested)

### Possible Consequences

Depending on the severity and nature of the violation, consequences may include:
- **Warning** for first-time minor infractions
- **Temporary suspension** from project participation
- **Permanent ban** from project spaces
- **Report to employer** for serious violations
- **Legal action** if required by law

## Conflict Resolution

### Disagreements and Technical Discussions

- **Focus on technical merits**, not personal opinions
- **Provide evidence** and reasoning for your position
- **Be open to alternative solutions**
- **Escalate to Trusted Committers** if needed

### Code Review Feedback

- **Be constructive** and specific in your feedback
- **Explain the "why"** behind your suggestions
- **Acknowledge good work** alongside improvement suggestions
- **Be receptive** to feedback on your own contributions

## Positive Contributions

We encourage behaviors that contribute to a healthy community:

### 🤝 Welcoming New Contributors
- **Answer questions** from newcomers patiently
- **Help with onboarding** and setup issues
- **Mentor** less experienced contributors
- **Celebrate** first contributions

### 📚 Knowledge Sharing
- **Document** your work clearly
- **Share best practices** and lessons learned
- **Create examples** and tutorials
- **Participate** in community discussions

### 🎯 Constructive Feedback
- **Focus on the code/content**, not the person
- **Suggest improvements** with specific examples
- **Acknowledge effort** and good intentions
- **Be open** to feedback on your own work

## Attribution

This Code of Conduct is adapted from the [Contributor Covenant](https://www.contributor-covenant.org), version 2.1, available at [https://www.contributor-covenant.org/version/2/1/code_of_conduct.html](https://www.contributor-covenant.org/version/2/1/code_of_conduct.html).

## Questions or Concerns?

If you have questions about this Code of Conduct or need clarification:
- **Email**: conduct@yourcompany.com
- **Slack**: Message any Trusted Committer
- **GitHub**: Open an issue with `code-of-conduct` label

---

## Our Commitment

We are dedicated to providing a safe, welcoming, and inclusive environment for all participants. We believe that diverse perspectives and experiences strengthen our project and community.

Thank you for helping us make AgentSkills a welcoming space for everyone! 🌟
