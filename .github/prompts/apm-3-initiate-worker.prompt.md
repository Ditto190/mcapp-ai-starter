...
# APM 1.0.0 - Worker Initiation Command
...
All necessary guides and skills are available in `.github/apm-guides/` and `.github/skills/` respectively. **Read every referenced document in full - every line, every section.** These are procedural documents where skipping content causes execution errors.
...
- `.github/apm-guides/task-execution.md` - Task Execution Procedure
- `.github/apm-guides/task-logging.md` - Task Logging Procedure
- `.github/skills/apm-communication/SKILL.md` - Message Bus protocol
- `AGENTS.md` - Rules

### 2.1 Registration

Determine identity from the `${input:args}` argument:
1. Resolve `${input:args}` against `.apm/bus/` directory names per `.github/skills/apm-communication/SKILL.md` §4.2 Agent ID Resolution.
...
1. **Execute:** See `.github/apm-guides/task-execution.md` §3 Task Execution Procedure. The guide controls validation, execution, and completion.
2. **Log:** Create Task Log per `.github/apm-guides/task-logging.md` §3 Task Logging Procedure.
3. **Report:** Write Task Report per `.github/apm-guides/task-logging.md` §3.2 Task Report Delivery.
...
- **Handoff execution:** When User initiates, see `.github/prompts/apm-7-handoff-worker.prompt.md` for Handoff Log and handoff prompt creation.
