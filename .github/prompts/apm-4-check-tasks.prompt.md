...
# APM 1.0.0 - Worker Check Tasks Command
...
1. Determine registration state:
   ...
   - If not registered, `${input:args}` is required. If no argument provided, inform User that an agent-id is required.

2. Resolve agent-id (unregistered Workers only): resolve `${input:args}` against `.apm/bus/` directory names per `.github/skills/apm-communication/SKILL.md` §4.2 Agent ID Resolution. Initialize per `.github/prompts/apm-3-initiate-worker.prompt.md` §2 Initiation.
...
4. Cross-validate `agent` field in YAML frontmatter against registered identity. Mismatch flags a routing error - decline and direct User to the correct Worker. Process the Task per `.github/apm-guides/task-execution.md` §3 Task Execution Procedure.
