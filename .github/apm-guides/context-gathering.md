# APM 1.0.0 - Context Gathering Guide
...
**Self-explore vs subagent:** For focused investigation (specific files, targeted questions, quick lookups), self-explore directly. For substantial research (cross-codebase exploration, extensive investigation), Spawn a dedicated exploration subagent for substantial research - it runs in its own context window, preserving yours for planning: `#runSubagent`. Structure the prompt with specific research questions and expected sources. Verify findings against the codebase, then present a summary to the User before incorporating into round reasoning.
...
3. Output procedure completion stating Context Gathering is complete. The next step is the Work Breakdown procedure per `.github/apm-guides/work-breakdown.md` §3 Work Breakdown Procedure.
