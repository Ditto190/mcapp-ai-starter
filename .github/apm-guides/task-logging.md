...
Two sequential steps after Task completion: write the Task Log, then deliver the Task Report via the bus. Execute after Task completion per `.github/apm-guides/task-execution.md` §3.6 Task Completion.
...
1. From the completion assessment already presented in chat per `.github/apm-guides/task-execution.md` §3.6 Task Completion, determine what to capture in the Task Log.
...
3. Direct the User to deliver the report to the Manager per `.github/skills/apm-communication/SKILL.md` §2.1 Direct Communication - provide both `/apm-5-check-reports <agent-id>` for targeted retrieval and `/apm-5-check-reports` as the general command, since multiple Workers may finish concurrently.
