...
**Investigation scope:** Investigate directly for contained checks; use a subagent for context-intensive issues. When scope is unclear, prefer subagent to preserve Manager context. When a subagent returns findings, verify critical claims by reading the key files it references before acting on them. Spawn a dedicated exploration subagent for investigation - it runs in its own context window, preserving yours for coordination: `#runSubagent`. Structure the prompt with the investigation goal, files to examine, and what to report back.

**Post-investigation outcome:**
- If no issues are found (false positives, nothing actionable), continue to the next Task(s).
- If the Worker needs to retry with refined instructions, create a follow-up Task Prompt per `.github/apm-guides/task-assignment.md` §3.4 Follow-Up Task Prompt Construction. If the Worker also left changes uncommitted, note this in the follow-up instructions.
...
**Wait state:** When no Tasks are Ready but Workers are active, communicate what was processed, what is pending, and which report(s) the User should return next. If a pending report would unlock a better dispatch combination per `.github/apm-guides/task-assignment.md` §2.4 Dispatch Standards, recommend the User prioritize that report.
...
2. Investigate and determine outcome per §2.2 Review Outcome Standards:
   ...
   - If the Worker needs a follow-up, create a follow-up Task Prompt per `.github/apm-guides/task-assignment.md` §3.4 Follow-Up Task Prompt Construction and continue to step 3.
...
4. Assess next action per §2.4 Parallel Coordination Standards:
   ...
   - If Tasks are Ready, proceed to `.github/apm-guides/task-assignment.md` §3.1 Dispatch Assessment in the same turn.
...
Write the end state of each Task for the review-dispatch cycle. When a Task is unblocked and dispatched in the same turn, write directly from Waiting to Active. When a Task is unblocked but cannot be dispatched - the assigned Worker has an Active Task or a pending report would unlock a better dispatch per `.github/apm-guides/task-assignment.md` §2.4 Dispatch Standards - write Ready.
...
- *`## Version Control`:* Per-repository base branch, branch convention, and commit convention per `.github/apm-guides/task-assignment.md` §4.4 Tracker VC Entry Format. Branch state is tracked per-Task in the Task table's Branch column.
