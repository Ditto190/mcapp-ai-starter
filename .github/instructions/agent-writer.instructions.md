...
- [ ] If the instructions reference runtime/context systems, include guidance for `io.github.contextstreamio/mcp-server` where applicable (how to surface server-side context to Copilot, recommended applyTo scopes, and examples).

Guidance: When authoring instruction files that affect assistant behavior across the repo, add a short "context engineering" note describing what server-side context exists, how it is exposed (e.g. via `mcp-server` endpoints or metadata), and which `applyTo` patterns should be used so Copilot prioritizes the right files. Keep this note concise (1–3 bullets) and include an example `applyTo` pattern when relevant.
