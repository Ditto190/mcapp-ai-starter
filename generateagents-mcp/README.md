...
1. **Start the MCP server:**
  ```bash
  python /workspaces/mcapp-ai-starter/generateagents-mcp/server.py
  ```
  Or with uv:
  ```bash
  cd /workspaces/mcapp-ai-starter/generateagents-mcp && uv run server.py
  ```
    ...
    "servers": [
      {
        ...
        "command": "python",
        "args": ["/workspaces/mcapp-ai-starter/generateagents-mcp/server.py"],
            "env": {
              "PYTHONPATH": "/workspaces/mcapp-ai-starter"
        }
      }
    ]
  }
