...
class InteractiveClient {
    ...
    async completion(kernelName, code, line, character) {
        ...
        let result = await this.submitCommandAndGetResult(command, commandsAndEvents_1.CompletionsProducedType, true);
        if (result === undefined) {
            result = {
                completions: []
            };
    ...
    async hover(language, code, line, character) {
        ...
        let result = await this.submitCommandAndGetResult(command, commandsAndEvents_1.HoverTextProducedType, true);
        if (result === undefined) {
            result = {
                content: []
            };
    ...
    async signatureHelp(language, code, line, character) {
        ...
        let result = await this.submitCommandAndGetResult(command, commandsAndEvents_1.SignatureHelpProducedType, true);
        if (result === undefined) {
            result = {
    ...
    async getDiagnostics(kernelName, code) {
        const command = new commandsAndEvents_1.KernelCommandEnvelope(commandsAndEvents_1.RequestDiagnosticsType, {
            code,
            targetKernelName: kernelName
        });
        let diagsProduced = await this.submitCommandAndGetResult(command, commandsAndEvents_1.DiagnosticsProducedType, true);
        if (diagsProduced === undefined) {
            return [];
        }
        return diagsProduced.diagnostics;
    }
    async submitCode(command, observer) {
    ...
    async requestValueInfos(kernelName) {
        ...
        const result = await (this.submitCommandAndGetResult(command, commandsAndEvents_1.ValueInfosProducedType));
        return result;
    }
    async requestCodeExpansionInfos() {
        const command = new commandsAndEvents_1.KernelCommandEnvelope(commandsAndEvents_1.RequestCodeExpansionInfosType, {
            targetKernelName: ".NET", // FIX look up actual root kernel
        });
        const result = await (this.submitCommandAndGetResult(command, commandsAndEvents_1.CodeExpansionInfosProducedType));
        return result;
    }
    async requestValue(valueName, kernelName) {
        const command = new commandsAndEvents_1.KernelCommandEnvelope(commandsAndEvents_1.RequestValueType, {
            name: valueName,
            mimeType: 'text/plain',
            targetKernelName: kernelName,
        });
        const result = await this.submitCommandAndGetResult(command, commandsAndEvents_1.ValueProducedType);
        return result;
    }
    cancel() {
