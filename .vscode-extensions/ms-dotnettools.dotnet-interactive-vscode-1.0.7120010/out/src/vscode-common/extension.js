...
exports.DotNetPathManager = exports.CachedDotNetPathManager = void 0;
...
async function activateCore(context, diagnosticsChannel) {
    ...
    const hasShownDeprecation = context.globalState.get(deprecationShownKey, false);
    ...
    await (0, commands_1.registerAcquisitionCommands)(context, diagnosticsChannel);
    // check sdk version
    let showHelpPage = false;
    try {
        const dotnetVersion = await (0, utilities_1.getDotNetVersionOrThrow)(exports.DotNetPathManager.getDotNetPath(), diagnosticsChannel);
    ...
    async function kernelChannelCreator(notebookUri) {
        var _a;
        const dotnetConfig = vscode.workspace.getConfiguration(constants.DotnetConfigurationSectionName);
        const polyglotConfig = vscode.workspace.getConfiguration(constants.PolyglotConfigurationSectionName);
        ...
        const environmentVariables = { ...polyglotConfig.get('kernelEnvironmentVariables'), 'DOTNET_CLI_CULTURE': getCurrentCulture(), 'DOTNET_CLI_UI_LANGUAGE': getCurrentCulture() };
    ...
    function configureKernel(compositeKernel, notebookUri) {
        ...
        compositeKernel.registerCommandHandler({
            ...
            handle: async (commandInvocation) => {
                ...
                (_a = vscode.window.activeNotebookEditor) === null || _a === void 0 ? void 0 : _a.revealRange(range);
    ...
    context.subscriptions.push(await (0, languageProvider_1.registerLanguageProviders)(clientMapper, languageServiceDelay));
...
async function openNotebookFromUrl(notebookUrl, notebookFormat, serializerMap, diagnosticsChannel) {
    ...
    try {
        ...
        const response = await (0, node_fetch_1.default)(notebookUrl);
...
async function waitForSdkPackExtension() {
...
async function getInteractiveLaunchOptions() {
    // use dotnet-interactive or install
    const installArgs = {
        dotnetPath: exports.DotNetPathManager.getDotNetPath(),
    };
    const launchOptions = await vscode.commands.executeCommand('dotnet-interactive.acquire', installArgs);
