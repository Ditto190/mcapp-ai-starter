...
export class ScaffoldCommand {
    ...
    async execute(targetPath: string | vscode.Uri, options?: ScaffoldOptions): Promise<void> {
        try {
            ...
            this.validateScaffoldTargetPath(targetUri.fsPath);
    ...
    /**
     * Validate target path before scaffolding.
     *
     * We require the top-level absolute directory (e.g. `/tmp`, `/home`) to
     * already exist so obviously invalid absolute paths fail deterministically
     * across environments.
     */
    private validateScaffoldTargetPath(targetPath: string): void {
        ...
        const isUnderAllowedRoot = allowedRoots.some(root => resolved === root || resolved.startsWith(`${root}${path.sep}`));

        if (!isUnderAllowedRoot) {
            throw new Error(`Cannot create directory outside allowed locations: ${resolved}`);
        }

        // Only enforce top-level existence for POSIX absolute paths.
        if (path.isAbsolute(resolved) && process.platform !== 'win32') {
            const parts = resolved.split(path.sep).filter(Boolean);
            if (parts.length > 0) {
                const topLevel = path.join(path.sep, parts[0]);
                if (!fs.existsSync(topLevel)) {
                    throw new Error(`Cannot create directory: top-level path does not exist (${topLevel})`);
                }
            }
        }
    }
