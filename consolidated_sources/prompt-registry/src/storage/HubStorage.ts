...
export class HubStorage {
    ...
    /**
     * Check if directory has write permission bits set.
     * This keeps behavior deterministic even when tests run under privileged users.
     */
    private ensureDirectoryWritable(dirPath: string): void {
        const dirStats = fs.statSync(dirPath);
        const hasWriteBit = (dirStats.mode & 0o222) !== 0;
        if (!hasWriteBit) {
            throw new Error(`Storage directory is not writable: ${dirPath}`);
        }
    }
    ...
    async saveHub(hubId: string, config: HubConfig, reference: HubReference): Promise<void> {
        ...
        try {
            this.ensureDirectoryWritable(this.storagePath);
